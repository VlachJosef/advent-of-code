...
maxMinDiff :: Array Int -> Maybe Int
maxMinDiff xs = (-) <$> maximum xs <*> minimum xs

solution1 :: SpreadSheet -> Maybe Int
solution1 = runSolution maxMinDiff

firstEvenlyDivide :: Array Int -> Maybe Int
firstEvenlyDivide xs = head do
  i <- xs
  j <- xs
  guard $ (i /= j) && (i `mod` j == 0)
  pure $ i / j

solution2 :: SpreadSheet -> Maybe Int
solution2 = runSolution firstEvenlyDivide

runSolution :: (Array Int -> Maybe Int) -> SpreadSheet -> Maybe Int
runSolution f spreadSheet = map (foldr (+) 0) (sequence $ map f spreadSheet)
